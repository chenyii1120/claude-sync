...
3. 🔌 複製插件資料（`CLAUDE.md`、`blocklist.json`、`data/`、插件專屬設定目錄）到 `repo/global/plugin-data/`，排除可重建的內容（`cache/`、`marketplaces/`）

4. 📂 複製 `~/.claude/commands/`、`~/.claude/rules/`、`~/.claude/agents/`、`~/.claude/skills/` 和 `~/.claude/hooks/` 到 `repo/user-config/`

5. 📤 執行 `git add -A && git commit && git push`
...
1. 💾 **自動備份** — 將目前的設定、插件設定、插件資料、commands、rules、agents、skills 和 hooks 快照到 `~/.claude/sync-backups/`（最多保留 5 份，自動清除最舊的）

2. 📤 **匯出並提交本地狀態** — 在合併前先將目前 `~/.claude/` 的狀態匯出並提交到 repo，確保 smart merge 能看到雙方的完整狀態，進行正確的三方合併
...
預覽本地與遠端之間的差異，**不會套用任何變更**。

顯示設定的欄位層級差異、插件設定的檔案層級差異，以及 user config（commands、rules、agents、skills、hooks）和插件資料的檔案層級狀態（modified/local-only/remote-only）。
...
還原設定、插件設定、插件資料、commands、rules、agents、skills 和 hooks。還原後自動重裝缺少的插件 — 與 `/sync-pull` 相同。
...
| `plugins/install-counts-cache.json` | 快取資料，可重建 |
