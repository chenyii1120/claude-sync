...
function exportPluginLock() {
  ...
  const existing = readJsonFile(destPath, { fallback: null });
  if (
    existing &&
...
function preparePinnedClone(name, url, commit, opts = {}) {
  ...
  if (!fs.existsSync(cloneDir)) {
    try {
      ...
      return { status: 'clone-failed', error: err.message };
    }
  }
...
// plugins.lock.json on this machine. For each selected marketplace: prepare
// a pinned local clone (preparePinnedClone), register it as a path-source
// marketplace under the same name (replacing any existing registration at a
// different installLocation), then reinstall every plugin that was either
// previously installed from that marketplace or is present in the lock, so
// nothing is left orphaned by the marketplace swap. Disabled plugins are
// re-disabled afterward, since install/update always re-enables.
function applyPluginLock(options = {}) {
  ...
  for (const name of selected) {
    ...
    const prep = preparePinnedClone(name, url, pinnedCommit, options.prepareOpts);
    if (prep.status !== 'ready') {
      results.push({ name, status: prep.status });
      if (prep.status === 'unreproducible') unreproducible.push(name);
      continue; // touch NO installs for a marketplace we couldn't prepare
    }

    // Snapshot before mutating anything, so the reinstall set accounts for
    // both what's actually installed right now and what the lock expects.
    ...
    results.push({ name, status: 'applied', commit: pinnedCommit, reinstalled: reinstallIds, disabled: disabledIds });
  }
