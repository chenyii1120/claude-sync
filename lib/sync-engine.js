...
function pinMarketplaceToCommit(name, url, commit, opts = {}) {
  ...
  const prep = preparePinnedClone(name, url, commit, opts.prepareOpts);
...
function setPinnedRef(name, ref, opts = {}) {
  ...
  let commit;
  try {
    commit = execFileSync('git', ['-C', cloneDir, 'rev-parse', `${ref}^{commit}`], { stdio: 'pipe' })
      .toString().trim();
  } catch {
