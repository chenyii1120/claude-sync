...
function exportPluginLock() {
  ...
  for (const id of enabledIds) {
    ...
    if (!(marketplace in marketplaceResults)) {
      ...
      // F#3 (fable review) url priority: (1) a CURRENT github source always
      // wins -- if known[marketplace].source is github right now, that's the
      // live truth and must be able to overwrite a stale previously-committed
      // url (the marketplace legitimately moved to a new repo); (2) else the
      // url already committed for this marketplace in the EXISTING lock --
      // preserves the original upstream across re-pushes from a machine that
      // has since applied the pin and whose known_marketplaces.json source is
      // now a local clone (the F#1 case, where source is NOT github); (3) the
      // pinned clone's own `origin` remote, which was cloned FROM the
      // original url; (4) the existing source.url fallback.
      ...
      let url = source && source.source === 'github'
...
function exportPluginVendorBundles() {
  ...
  // <name> off`) -- including the case where the list is now EMPTY (the
  // last vendored marketplace was turned off). This must run BEFORE the
  // early returns below, or de-vendoring the only/last marketplace would
  // never clean up its leftover dir. Removing only subdirs whose name is
  // NOT in `names` is safe in every case: an empty `names` prunes all of
  // them, a non-empty `names` keeps the still-vendored ones untouched.
  const vendorRoot = path.join(REPO_DIR, 'global', 'plugin-vendor');
  // F#-fable MED: a synced repo an attacker controls could commit
  // global/plugin-vendor (or a per-marketplace dir under it) AS A SYMLINK --
  // `reset --hard` materializes it, and writing/mkdir-ing into it would then
  // write THROUGH the link to an attacker-chosen directory. Neutralize the
  // root before even listing it, same F#9 primitive used elsewhere in this
  // file; unlinkIfSymlink() is a no-op on a real directory.
  unlinkIfSymlink(vendorRoot);
  ...
  for (const name of names) {
    ...
    // F#-fable MED: neutralize a symlinked per-marketplace dir BEFORE the
    // existsSync/readdirSync below -- otherwise a planted `plugin-vendor/mp`
    // symlink would make the "stale bundle" prune loop list and delete
    // entries in the attacker-chosen external target instead.
    unlinkIfSymlink(outDir);
...
function pinMarketplaceToCommit(name, url, commit, opts = {}) {
  ...
  // F#-fable HIGH: a registration failure (e.g. the CLI rejecting the
  // path-source add) must not propagate -- it would otherwise abort
  // applyPluginLock()'s marketplace loop and skip every remaining
  // marketplace. Report it instead and touch no installs for this one.
  try {
    ...
    return { name, status: 'register-failed', error: err.message };
  }
  ...
  // F#-fable HIGH: reinstallIds intentionally includes lock-external local
  // plugins (F#8), which may not exist at an older pinned commit -- their
  // install can legitimately fail. Wrap each install so one bad id doesn't
  // abort the loop (leaving it uninstalled-but-not-reinstalled) or skip the
  // disable re-apply loop below.
  const failed = [];
  ...
  return { name, status: 'applied', commit, reinstalled: reinstallIds, disabled: disabledIds, failed };
}
...
function applyPluginLock(options = {}) {
  ...
  for (const name of selected) {
    ...
    // F#-fable HIGH: one marketplace throwing (an unexpected error out of
    // preparePinnedClone/pinMarketplaceToCommit) must not abort the loop --
    // record it and move on to the remaining marketplaces.
    let result;
    try {
      result = pinMarketplaceToCommit(name, url, pinnedCommit, {
        runPlugin, prepareOpts: options.prepareOpts, lockedIds,
      });
    } catch (err) {
      result = { name, status: 'error', error: err.message };
    }
...
function reverseMigrateMarketplace(name, opts = {}) {
  ...
  // F#-fable HIGH parity: a plugin that no longer exists at the marketplace's
  // latest (e.g. renamed/removed upstream) can legitimately fail to install.
  // Wrap it so one bad id doesn't abort the loop or skip the disable
  // re-apply loop and clone-dir cleanup that follow.
  const failed = [];
  ...
  return { name, status: 'unpinned', reinstalled: priorInstalledIds, disabled: disabledIds, failed };
}
