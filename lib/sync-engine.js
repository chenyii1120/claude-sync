...
// Read file contents from a git ref. Unlike gitExec, this does NOT trim the
// output — file content compare must preserve trailing newlines or the
// caller will see false-positive diffs against the working tree.
function safeGitShow(ref, filePath) {
  try {
    return execSync(`git show ${ref}:${filePath}`, {
      cwd: REPO_DIR, timeout: 30000, stdio: 'pipe',
    }).toString();
  } catch {
    return null;
  }
...
function init(remoteUrl) {
  ...
  // than try to merge.
  const commitHash = gitExec('rev-parse HEAD');
...
function push() {
  ...
  try {
    ...
    saveLastSync({ action: 'push', commitHash: gitExec('rev-parse HEAD'), firstPullPending: false });
...
function previewPull() {
  ...
  const lastSync = loadLastSync();
  const remoteHead = gitExec('rev-parse origin/main');
...
function pull(options = {}) {
  ...
  try {
    ...
    try { gitExec('reset --hard origin/main'); } catch {}
