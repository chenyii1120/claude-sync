...
// (rev-list HEAD..origin/main), not a hash mismatch -- HEAD != origin/main
...
function importSettings(options = {}) {
  ...
  const remote = transformPathsForImport(JSON.parse(fs.readFileSync(repoSettings, 'utf8')));
...
// origin/main by pull() before importAll() runs) -- i.e. the remote side
...
function pull(options = {}) {
  ...
  try {
    ...
    if (nothingChanged) {
      return { pulled: false, reason: 'up-to-date', backupPath, ...result, missingPlugins, mode: modeUsed, pendingConfirmation, unknownRemoteDirs, suspiciousRemoteDirs };
    }
    return { pulled: true, backupPath, ...result, missingPlugins, mode: modeUsed, pendingConfirmation, unknownRemoteDirs, suspiciousRemoteDirs };
  } finally {
    releaseLock();
  }
}
