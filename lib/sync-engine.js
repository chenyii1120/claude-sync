...
function push() {
  ...
  try {
    ...
    return { pushed: true, mergeConflicts: allConflicts };
  } finally {
    releaseLock();
  }
}
...
function pull() {
  ...
  try {
    ...
    for (const file of MERGE_JSON_FILES) {
      ...
      const r = safeGitShow('origin/main', file);
      if (b != null && l != null && r != null) {
        const m = mergeJsonFields(JSON.parse(b), JSON.parse(l), JSON.parse(r), 'remote');
        mergedFiles[file] = m.result;
        allConflicts.push(...m.conflicts);
      }
    }
    ...
    try {
      ...
      gitExec('merge --abort');
      gitExec('merge origin/main --no-edit -X theirs');
    }
    ...
    return { pulled: true, backupPath, ...result, missingPlugins, mergeConflicts: allConflicts };
  } finally {
    releaseLock();
  }
}
