...
function listDirsAtRef(ref, prefix) {
  ...
  for (const line of output.split('\n')) {
    ...
    if (name && !SYSTEM_EXCLUDE_DIRS.has(name)) dirs.add(name);
  }
  return dirs;
}
...
function getSyncDirsForImport() {
  ...
  return fs.readdirSync(repoUserConfig, { withFileTypes: true })
    ...
    .map(e => e.name)
    .filter(d => allow.has(d) && !SYSTEM_EXCLUDE_DIRS.has(d));
}
...
// already declined), matching export/diff behaviour.
function getUnknownRemoteDirs() {
  ...
  return fs.readdirSync(repoUserConfig, { withFileTypes: true })
    .filter(e => e.isDirectory())
    .map(e => e.name)
    .filter(d => !allow.has(d) && !skip.has(d) && !SYSTEM_EXCLUDE_DIRS.has(d));
}
...
// C-01: `ref` defaults to origin/main (fetched-but-not-pulled remote state)
// rather than REPO_DIR's working tree (= local clone's HEAD as of the last
// pull/push). No internal call site needs the old HEAD-comparing behaviour
// -- pass ref='HEAD' explicitly if that ever changes.
function diffSettings(ref = 'origin/main') {
...
// C-01: see diffSettings() above -- `ref` defaults to origin/main.
function diffPluginConfigs(ref = 'origin/main') {
  const diffs = [];
  for (const file of ['installed_plugins.json', 'known_marketplaces.json']) {
    const remoteRaw = safeGitShow(ref, `global/${file}`);
    if (remoteRaw == null) continue;
    const remote = transformPathsForImport(JSON.parse(remoteRaw));
...
// C-01: see diffSettings() above -- `ref` defaults to origin/main. The repo
// side is read via listFilesAtRef()/safeGitShow() at `ref` instead of
// walking REPO_DIR's working tree.
function diffUserConfig(ref = 'origin/main') {
  const diffs = [];
  ...
  for (const dir of dirs) {
    ...
    for (const file of allFiles) {
      ...
      if (inRepo && inLocal) {
        const repoContent = safeGitShow(ref, `${repoPrefix}/${file}`);
...
// C-01: see diffSettings() above -- `ref` defaults to origin/main.
function diffPluginData(ref = 'origin/main') {
...
// origin/main by pull()) against ~/.claude. Returns
...
function pull(options = {}) {
  ...
  try {
    ...
    // working tree (reset to origin/main above) read-only, so the preview and
    // the exclusion see the same source.
    ...
    if (nothingChanged) {
      return { pulled: false, reason: 'up-to-date', backupPath, ...result, missingPlugins, mode: modeUsed, pendingConfirmation, unknownRemoteDirs };
    }
    return { pulled: true, backupPath, ...result, missingPlugins, mode: modeUsed, pendingConfirmation, unknownRemoteDirs };
  } finally {
    releaseLock();
  }
}
...
// stashed by pull() and clears the pending state. Does NOT re-backup — pull()
...
//
// NOTE: applying only a subset advances last-sync to the confirmed commit; any
// unapplied executable dir will then read as a local change on the next pull.
// The skill flow confirms/discards all pending dirs in the same run, so this is
// an edge case, not the normal path.
function applyPendingDirs(dirs) {
  ...
  try {
    ...
    if (commitHash) { try { gitExecFile(['reset', '--hard', commitHash]); } catch {} }
    ...
    clearPendingApply();
    return { applied: true, dirs: toApply, configChanges };
  } finally {
    releaseLock();
  }
}

// B-02 stage two (decline): drop the pending state WITHOUT applying and WITHOUT
...
// same dirs under `pendingConfirmation`.
...
module.exports = {
  ...
  // Sync-dir resolution
  getSyncDirsForExport, getSyncDirsForImport, getUnknownRemoteDirs, detectUnknownDirs,
  ...
  listFilesRecursive, listFilesAtRef, listDirsAtRef, getStatus,
  ...
  // Orchestrators
  init, push, pull, applyPendingDirs, discardPendingDirs, uninstall,
};
