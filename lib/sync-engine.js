...
function push() {
  ...
  try {
    ...
    // B-04: warn (never block) if settings.json's env looks like it holds
    // secrets -- fallback:{} is safe here since exportAll() above already
    // required settings.json to parse cleanly; this is just re-reading the
    // known-good local file (pre A-04 path transform, so values are intact).
    ...
    return { pushed: true, ...mergeResult, secretWarnings };
  } finally {
    releaseLock();
  }
}
