...
function getStatus() {
  ...
  try {
    ...
    // C-04: exportAll() -> exportSettings() now fails fast (SOURCE,
    // required:true) on a corrupt local settings.json. getStatus() is
    // read-only and must still render -- downgrade to a warning instead of
